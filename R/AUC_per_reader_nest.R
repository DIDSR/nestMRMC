    ...
    for (r in 1:nr) {
      ...
      sum_comb = function(index){
        ...
        if (ma == 0 | nb == 0) {
          ...
          temp_m1 = matrix(this_neg,nrow = ma,ncol = nb,byrow = T)
          temp_m2 = matrix(this_pos,nrow = ma,ncol = nb,byrow = F)
      ...
      success_array[m,r,,,1] = temp_out[1,] %>% matrix(np,np,byrow = T)
      success_array[m,r,,,2] = temp_out[2,] %>% matrix(np,np,byrow = T)
